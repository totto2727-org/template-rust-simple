...
- Keep `package.nix` and the package and overlay outputs for distributable CLI applications so every documented Nix path remains valid; remove them only when the copied project is no longer distributed as a CLI.
...
- In the generated README, keep Usage focused on the installed command and document direct `nix run`, the applicable crates.io or Git `cargo install`, `nix profile install`, and declarative overlay-based `flake.nix` setup as mutually exclusive choices; state that only one setup method is required.
